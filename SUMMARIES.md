# awesome-cobol — Link Summaries

Each entry gives a brief description of the resource and a note on its relevance to COBOL developers.
...
**[snek (Snake)](https://github.com/lewisjb/snek)** — A basic Snake game written in COBOL using the ncurses library for terminal graphics on a 10x10 grid. *COBOL relevance: Illustrates how GnuCOBOL can call C-based terminal libraries like ncurses for real-time keyboard input and screen rendering.*
...
**[CICS GenApp (Insurance Software)](https://github.com/cicsdev/cics-genapp)** — IBM's General Insurance Application (originally SupportPac CB12), a working COBOL/JCL application on CICS TS for z/OS that manages insurance policy data in Db2 and demonstrates how traditional 3270 green-screen applications can be modernized. *COBOL relevance: A well-documented, intermediate-level reference application that illustrates CICS API usage, Db2 integration, and application modernization patterns directly relevant to mainframe COBOL developers.*
...
**[Re-Boot Hill](https://web.archive.org/web/20110908081646/http://www.actscorp.com/reboothill.htm)** — A humor page from ACTS Corporation featuring tongue-in-cheek tombstones and epitaphs for retired mainframe technologies and programming languages. *COBOL relevance: Light-hearted industry humor that celebrates — and gently mocks — the longevity of COBOL and its mainframe companions.*
...
> **Note:** The `unstring` SourceForge link resolves to the **Coboldoc** project — the project was renamed and the link text no longer matches. `cobol-dde` has been inactive since 2013 and was folded into the `stingrayreader` project.
